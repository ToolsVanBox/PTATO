...
if ( prec_rec$precision < prec_cutoff ) {
  PTAprob_cutoff <- as.numeric(PTAprob_cutoff_walker)
} else {
  PTAprob_cutoff <- as.numeric(c(PTAprob_cutoff_cluster,PTAprob_cutoff_walker))
}

cat( mean(PTAprob_cutoff), PTAprob_cutoff )
